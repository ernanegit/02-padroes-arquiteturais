...
class Application {
  ...
  public async initialize(): Promise<void> {
    try {
      // PRIMEIRO: Initialize dependency injection container
      await this.container.initialize();
      this.logger.info('Dependency injection container initialized');

      // SEGUNDO: Initialize database connection
      await DatabaseConnection.connect();
      this.logger.info('Database connected successfully');

      // TERCEIRO: Initialize Redis connection
      await RedisConnection.connect();
      this.logger.info('Redis connected successfully');

      // QUARTO: Initialize middlewares
      this.initializeMiddlewares();
      this.logger.info('Middlewares initialized');

      // QUINTO: Initialize routes (só depois do container estar pronto)
      this.initializeRoutes();
      this.logger.info('Routes initialized');

      // SEXTO: Initialize error handling
